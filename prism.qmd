...
* If you eliminate `-m prod` from the second command, it starts the server in test mode (does not actually send mesages)
...
* NOTE these are initially in `optimize/automation/dev` until reviewed & moved by John.
* I've ordered them by the order in which they must be used, ie. we have to download qualtrics data before we can add participants to followmee or prism.

1. optimize/automation/get_qualtrics.R: Download qualtrics surveys including screen, guid pedigree, baseline, ema, feedback, bimonthly
   a. Input: `S:/other/credentials.csv`
   b. Output: `S:/optimize/data_raw/surveys/` (one csv file for each distinct survey)
   c. Run 4x/day at 02:00 (to build feedback), 07:58 (to check for reminders), 14:00 (to build ema), 17:58 (to check for reminders)

2. optimize/automation/mak_reminders.R: Checks latest EMA and feedback downloads and create a PRISM config file for who needs a reminder
   a. Input: `S:/optimize/data_raw/participants/study_participants.csv`, `S:/optimize/data_raw/surveys/ema.csv`, `S:/optimize/data_raw/surveys/feedback.csv`
   b. Output: `C:/github/prism/prism/config/reminders.csv`
   c. Run 2x day at 08:00:00 and 18:00:00
   
2. optimize/automation/update_participants.R: 
   a. Input: `S:/other/credentials.csv`, `S:/optimize/data_raw/surveys/guid_pedigree.csv`, `S:/optimize/data_raw/surveys/onboard.csv`, `S:/optimize/data_raw/surveys/session_form.csv`, `S:/optimize/data_raw/surveys/wog.csv`
   b. Output: `S:/optimize/data_raw/participants/study_participants.csv`, subset of columns to`C:/github/prism/prism/config/study_participants.csv` for use in PRISM.
   c. Run 4x day at 00:05, 06:05, 12:05, 18:05.
   d. **NEEDS CODING** but before that need to finalize the session form and wog qualtrics surveys

3. optimize/automation/make_tone_style.R: Half of participants will get their selected tone/style per baseline survey, half will be yoked to an existing tone/style. QUESTION: I think this should be renamed make_conditions and also track of which of the 2x2x2x2 conditions each participant is assigned to, and tone/style are just part of that.
   a. Input: `S:/optimize/data_raw/surveys/baseline.csv`
   b. Output: `S:/optimize/data_processed/tone_style_preferences.csv` (Aggregate across all subids)
   c. Run 1x day
   d. **NEEDS CODING** 

4. optimize/automation/get_followmee.R: 
   a. Input: `S:/other/credentials.csv`
   b. Output: `S:/optimize/data_raw/$subid/gps/` (daily subid level snapshots), `S:/optimize/data_raw/$subid/gps_$subid.csv` (combined subid level file of all sub GPS), `S:/optimize/data_processed/gps.csv` (all participant GPS)
   c. Run 1x day at 00:00
   d. Status: JC approved

5. optimize/automation/find_new_places.R: Detect new locations for each subid, 
   a. Input: S:/optimize/data_raw/
   b. Output: `S:/optimize/data_raw/$subid/places_$subid.csv` 
   c. Run 4x day at 00:40, 06:40, 12:40, 18:40.
   d. **ready for Review**
   e. **FUTURE UPDATE**: Should also check downloaded ema.csv and updates places file for each participant with any newly provided context. 

6. optimize/automation/manage_contacts.R: Script to update qualtrics contact list for use with EMA/feedback surveys. Update as participants are added/removed and also update daily with gps locations which need context for on-study participants
   a. Input: `S:/optimize/data_raw/`
   b. Output: `S:/optimize/data_raw/participants/qualtrics_contact_list/optimize_contacts.csv` 
   c. Run 4x day at 00:40, 06:40, 12:40, 18:40.
   d. **NEEDS CODING**

7. optimize/automation/make_predictions.R:  
  a. Input: `S:/optimize/data_raw/surveys/ema.csv`, `S:/optimize/data_processed/gps.csv`
  b. Output: `S:/optimize/data_raw/$subid/predictions_$subid.csv` (subid level, one row per day of their predictions, features, etc)
  c. Run 1x day
  d. **NEEDS CODING**
  
7. optimize/automation/make_feedback.R: Script to create the daily support/feedback message for each participant based on their prediction file - creates the daily message
  a. Input: `S:/optimize/data_raw/$subid/predictions_$subid.csv` , `S:/optimize/data_processed/tone_style.csv` (or `conditions.csv`)
  b. Output: `S:/optimize/data_raw/$subid/predictions_$subid.csv` (subid level, every day has a new row for feedback)
   c. Run 1x day
   d. **NEEDS CODING**
...
d.	4: RUN_R_SCRIPT @ 05:40:00 download_qualtrics.R - Run Today: True

You also have the option to remove and add tasks.

To add tasks:

* open the script_pipeline.csv file in C:\github\prism\prism\config. Manually add the task by the name of the script. 
...
* System_task_schedule.csv  – names of scripts to be run along with run configs and times – see prism\scripts\README.md and SOP for Adding scripts to PRISM
